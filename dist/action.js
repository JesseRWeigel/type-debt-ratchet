...
import { resolvePullRequestNumber, upsertComment } from "./github.js";
import { renderComment, renderTerminal, verdictLine } from "./report.js";
import { DEFAULT_OPTIONS, EXIT_NEW_DEBT, EXIT_USAGE, runRatchet } from "./run.js";
import { TscRunError } from "./tsc.js";
...
export function booleanInput(env, name, fallback) {
...
async function appendSummary(env, markdown) {
...
export async function run(env = process.env) {
    ...
    if (booleanInput(env, "comment", true)) {
        ...
        if (prNumber === null) {
            ...
            process.stderr.write("::warning::comment is enabled but github-token or GITHUB_REPOSITORY is missing, skipping the comment.\n");
        }
        else {
            try {
                ...
                process.stderr.write(`::warning::could not post the PR comment: ${error.message}\n`);
            }
        }
    }
