...
import type { Baseline, BaselineEntry, Signature, SignatureMode } from "./types.ts";
...
export function buildBaseline(
  counts: ReadonlyMap<string, { signature: Signature; count: number }>,
  mode: SignatureMode,
): Baseline {
...
export function emptyBaseline(mode: SignatureMode): Baseline {
...
export function parseBaseline(text: string, source: string): Baseline {
  ...
  try {
    ...
    throw new BaselineError(`${source} is not valid JSON: ${(error as Error).message}`);
  }
  ...
  if (record["version"] !== 1) {
    throw new BaselineError(
      `${source} has version ${JSON.stringify(record["version"])}, this tool understands version 1`,
    );
  }
  ...
  const entries: BaselineEntry[] = rawEntries.map((raw, index) => {
  ...
  if (typeof declaredTotal === "number" && declaredTotal !== actualTotal) {
    throw new BaselineError(
      `${source} says totalErrors is ${declaredTotal} but its entries sum to ${actualTotal}. ` +
...
export async function writeBaseline(filePath: string, baseline: Baseline): Promise<boolean> {
