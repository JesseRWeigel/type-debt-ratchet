 ...
 */
export function normalizeMessage(message: string, mode: SignatureMode = "loose"): string {
  const collapsed = message.replace(/\s+/g, " ").trim();
  if (mode === "exact") return collapsed;
  return collapsed
    .replace(/'[^']*'/g, QUOTED)
 ...
 */
export function hashSignature(file: string, code: string, message: string): string {
 ...
 */
export function countSignatures(signatures: readonly Signature[]): Map<string, { signature: Signature; count: number }> {
  const counts = new Map<string, { signature: Signature; count: number }>();
