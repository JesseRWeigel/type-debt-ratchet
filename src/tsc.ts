...
import { looksLikePrettyOutput, parseTscOutput } from "./parse.ts";
...
export async function runTypeCheck(
  command: string,
  cwd: string,
  runner: (command: string, cwd: string) => Promise<{ output: string; exitCode: number }> = runCommand,
): Promise<TscResult> {
  ...
  if (diagnostics.length === 0 && exitCode !== 0) {
    if (looksLikePrettyOutput(output)) {
      throw new TscRunError(
        `The type-check command produced pretty-printed output, which cannot be parsed reliably.\n` +
          `Add --pretty false to the command (currently: ${command}).`,
      );
    }
    throw new TscRunError(
      `The type-check command exited ${exitCode} but produced no parseable diagnostics.\n` +
