...
import type { DebtChange, RatchetDiff } from "./types.ts";
...
function markdownTable(title: string, changes: readonly DebtChange[]): string {
  if (changes.length === 0) return "";
  const shown = changes.slice(0, MAX_ROWS);
  const rows = shown.map(
    (change) =>
      `| \`${cell(change.file || "(project)")}\` | ${change.code} | ${change.baselineCount} → ${change.currentCount} | ${signed(change.delta)} | ${cell(truncate(change.message, 120))} |`,
  );
...
export function verdictLine(diff: RatchetDiff): string {
...
export function renderComment(diff: RatchetDiff, options: { readonly baselinePath: string }): string {
  ...
  if (diff.renames.length > 0) {
    parts.push(
      ...
      ...diff.renames.map(
        (rename) =>
          `- \`${rename.from}\` → \`${rename.to}\` (${rename.errors} existing error${rename.errors === 1 ? "" : "s"} carried over)`,
      ),
    );
  }

  const addedTable = markdownTable("New debt", diff.added);
  if (addedTable !== "") parts.push("", addedTable);

  const fixedTable = markdownTable("Debt paid down", diff.fixed);
  if (fixedTable !== "") parts.push("", fixedTable);

  if (diff.newErrorCount > 0) {
    parts.push(
      "",
      "Fix the errors above, or if they are intentional, record them with:",
      "",
      "```bash",
    ...
    parts.push(
      "",
      `\`${options.baselinePath}\` is now ahead of reality. Run \`npx type-debt-ratchet --update-baseline\` to bank the fixes so they cannot come back.`,
    );
  }
...
export function renderTerminal(diff: RatchetDiff, options: { readonly baselinePath: string }): string {
  const lines: string[] = [];
  lines.push(
    `baseline ${diff.baselineTotal}  current ${diff.currentTotal}  new ${diff.newErrorCount}  fixed ${diff.fixedErrorCount}`,
  );
  ...
  if (diff.added.length > 0) {
    ...
    for (const change of diff.added.slice(0, MAX_ROWS)) {
      lines.push(
        `  ${signed(change.delta)}  ${change.file || "(project)"}  ${change.code}  ${truncate(change.message, 100)}`,
      );
    }
